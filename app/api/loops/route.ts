...
export async function POST(req: NextRequest) {
    try {
        console.log('📝 Loop creation request received - checking auth via refreshToken cookie...');
        ...
        if (!validationResult.success) {
            ...
            return NextResponse.json(
                { error: 'Invalid request payload', issues: validationResult.error.issues },
                { status: 400 }
            );
        }
        ...
        console.log('✅ Loop created successfully:', loop?._id ? loop._id.toString() : 'ID unknown');
