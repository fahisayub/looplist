...
function formatFrequency(frequency: string | string[]): string {
    if (typeof frequency === 'string') {
        ...
        return frequency.map(day => day.charAt(0).toUpperCase() + day.slice(1, 3)).join(', ');
    }
    return 'Custom';
}

// Get status based on current streak
function getLoopStatus(currentStreak: number): { label: string; color: string; emoji: string } {
    if (currentStreak > 0) {
        return {
            label: 'Active',
    ...
    return (
        <div className="border rounded-lg p-5 bg-card hover:shadow-md transition-shadow">
            ...
            <div className="mt-3 text-sm text-muted-foreground space-y-1">
                <p><strong>Frequency:</strong> {formatFrequency(loop.frequency)}</p>
                <p><strong>Started:</strong> {formatDistanceToNow(new Date(loop.startDate), { addSuffix: true })}</p>
                <p><strong>Visibility:</strong> {loop.visibility.charAt(0).toUpperCase() + loop.visibility.slice(1)}</p>
            </div>

            <div className="mt-4 pt-3 border-t flex justify-between items-center">
                <div className="text-sm">
                    <p><strong>Current streak:</strong> {loop.currentStreak || 0} days</p>
                    <p><strong>Longest streak:</strong> {loop.longestStreak || 0} days</p>
                </div>
                ...
                <div className="flex gap-2">
                    ...
                    <button
                        ...
                        className={`px-3 py-1 rounded-md text-sm transition-all ${checkInStatus === 'success'
