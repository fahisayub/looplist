        ...
        immer((set, get) => ({
            ...
            submitCheckIn: async (loopId: string, date?: Date) => {
                ...
                try {
                    ...
                    set((state) => {
                        ...
                        const exists = state.checkIns.some(c =>
                            c.loopId.toString() === loopId &&
                            new Date(c.date).toDateString() === new Date(data.checkIn.date).toDateString()
                        );
                        ...
                        if (state.loopStats[loopId]) {
                            ...
                            const dateStr = new Date(data.checkIn.date).toISOString().split('T')[0];
                            ...
                            if (state.loopStats[loopId].expectedCheckIns > 0) {
                                state.loopStats[loopId].completionRate = Math.round(
                                    (state.loopStats[loopId].totalCheckIns / state.loopStats[loopId].expectedCheckIns) * 100
                                );
                            }
                        }
            ...
            deleteCheckIn: async (loopId: string, checkInDate: Date) => {
                ...
                try {
                    ...
                    const response = await fetch(`/api/loops/${loopId}/checkin/${dateParam}`, {
                        method: 'DELETE'
                    });
                    ...
                    set((state) => {
                        ...
                        if (state.loopStats[loopId]) {
                            ...
                            state.loopStats[loopId].totalCheckIns = Math.max(0, state.loopStats[loopId].totalCheckIns - 1);
