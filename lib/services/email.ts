...
const useMockEmail = isDevelopment && !process.env.RESEND_API_KEY && !process.env.GMAIL_APP_PASSWORD;
const useNodemailer = process.env.USE_NODEMAILER === 'true' || (!process.env.RESEND_API_KEY && process.env.GMAIL_APP_PASSWORD);
...
function mockResend() {
  console.warn('Using mock email service for development. Emails will be logged to console.');
